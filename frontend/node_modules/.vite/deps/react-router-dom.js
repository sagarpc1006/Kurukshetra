...
function createRouter(init) {
	...
	async function handleAction(request, location, submission, matches, isFogOfWar, opts) {
		...
		if (!actionMatch.route.action && !actionMatch.route.lazy) result = {
			...
			result = (await callDataStrategy("action", request, [actionMatch], matches))[0];
	...
	async function handleFetcherAction(key, routeId, path, match, requestMatches, isFogOfWar, flushSync, submission) {
		...
		let actionResult = (await callDataStrategy("action", fetchRequest, [match], requestMatches))[0];
		...
		if (future.v7_fetcherPersist && deletedFetchers.has(key)) {
			...
			if (isRedirectResult(actionResult)) {
				...
				if (pendingNavigationLoadId > originatingLoadId) {
					...
					return startRedirectNavigation(fetchRequest, actionResult, { fetcherSubmission: submission });
				}
			}
	...
	async function handleFetcherLoader(key, routeId, path, match, matches, isFogOfWar, flushSync, submission) {
		...
		fetchControllers.set(key, abortController);
		let originatingLoadId = incrementingLoadId;
		let result = (await callDataStrategy("loader", fetchRequest, [match], matches))[0];
	...
	*/
	...
	async function callLoadersAndMaybeResolveData(currentMatches, matches, matchesToLoad, fetchersToLoad, request) {
		let [loaderResults, ...fetcherResults] = await Promise.all([matchesToLoad.length ? callDataStrategy("loader", request, matchesToLoad, matches) : [], ...fetchersToLoad.map((f) => {
			if (f.matches && f.match && f.controller) return callDataStrategy("loader", createClientSideRequest(init.history, f.path, f.controller.signal), [f.match], f.matches).then((r) => r[0]);
	...
	function checkFogOfWar(matches, routesToUse, pathname) {
		if (patchRoutesOnMissImpl) {
			if (!matches) return {
				...
				if (leafRoute.path && (leafRoute.path === "*" || leafRoute.path.endsWith("/*"))) return {
					active: true,
					matches: matchRoutesImpl(routesToUse, pathname, basename, true)
				};
			}
	...
	async function discoverRoutes(matches, pathname, signal) {
		...
		while (true) {
			...
			try {
				...
				if (isNonHMR) dataRoutes = [...dataRoutes];
			}
			...
			if (newMatches) {
				...
				if (leafRoute.index) return {
					type: "success",
					matches: newMatches
				};
				if (leafRoute.path && leafRoute.path.length > 0) {
					if (leafRoute.path === "*") matchedSplat = true;
					else return {
						type: "success",
						matches: newMatches
					};
...
function processLoaderData(state, matches, matchesToLoad, results, pendingActionResult, revalidatingFetchers, fetcherResults, activeDeferreds) {
	let { loaderData, errors } = processRouteLoaderData(matches, matchesToLoad, results, pendingActionResult, activeDeferreds, false);
...
function useViewTransitionState(to, opts) {
	...
	vtContext ?? invariant(false, "`unstable_useViewTransitionState` must be used within `react-router-dom`'s `RouterProvider`.  Did you accidentally import `RouterProvider` from `react-router`?");
